...
export class MachineService {
  ...
  async syncMachine(machineId: string) {
    return axiosInstance.post(`/api/pool/evm/${machineId}/sync`, {}, {
      headers: {
  ...
  async getMachineList(ownerAddress: string, statuses: PoolStatus[]): Promise<PoolEntity[]> {
  ...
  async syncWalletMachines(walletAddress: string) {
    return axiosInstance.post(`/api/pool/user/evm/${walletAddress}/sync`, {}, {
      headers: {
        "content-type": "text/plain;charset=UTF-8",
      },
      params: {
